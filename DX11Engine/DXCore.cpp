...
DXCore::~DXCore()
{
	...
	if (pbrRTV) { pbrRTV->Release(); }
	if (pbrSRV) { pbrSRV->Release(); }
...
HRESULT DXCore::InitDirectX()
{
	...
	ID3D11Texture2D* pbrBufferTexture;
	device->CreateTexture2D(&bufferDesc, 0, &pbrBufferTexture);
	device->CreateRenderTargetView(
		pbrBufferTexture,
		0,
		&pbrRTV);
